...
function ItemText ({ title, image, description, price, highlight = false }) {
    ...
    return (
        <article>
            ...
            <Slide className={`px-2 py-2 w-full ${highlight ? classes.productContainerHighLightStyle : ''}`} direction='up' cascade triggerOnce >
                <div className={`${classes.productContainerStyle}`}>
                    <div className={`flex flex-row ${classes.productTitleStyle}`} onClick={() => setShowModalImage(true)}>
                    ...
                    <p className={`text-sm ${classes.priceDescriptionStyle}`} >{description}</p>
                    <div className={`${classes.priceDescriptionStyle}`}>
                        {price.map((p, index) => {
                            return <span key={index + ' '} className='text-xs mr-5'>{p.price}</span>;
                        })}
                    </div>
                </div>
            </Slide>
