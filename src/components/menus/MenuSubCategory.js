...
function RenderAs ({ items, renderAs = 'text' }) {
    ...
    case 'text':
        return items.map(item => {
            return (
                <ItemText key={item.id} title={item.title} description={item.description} price={item.price} highlight={item.highlight} image={item.image} />
            );
        });
    case 'card':
        return items.map(item => {
            return <ItemCard key={item.id} title={item.title} description={item.description} price={item.price} image={item.image} />;
        });
    default:
...
function MenuSubCategory ({ title, description, items, image }) {
    ...
    return (
        <>
            ...
            <section className='mt-1 mb-5 mx-5 md:mx-5 md:mt-5 md:mb-10 flex flex-col content-center justify-center' style={theme.category.subCategoryContainerStyle}>
                <h1 className='font-bold subpixel-antialiased text-4xl text-center' style={theme.category.subCategoryHeadingStyle}>{title}</h1>
