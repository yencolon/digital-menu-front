...
function MenuNavigation({ match, containerStyle, itemStyle, hasDelivery = true }) {
    ...
    return (
        <nav className="bottom-0 fixed w-full lg:hidden flex items-center justify-around flex-wrap p-2 text-white" style={containerStyle}>
            <NavLink to={`${url}`} className='flex flex-col text-sm w-3/12 text-center items-center justify-center font-bold'>
                {/* <HomeOutlined style={{ fontSize: '20px' }} /> */}
                <Rest />
                Menú
            </NavLink>

            <NavLink to={`${url}/payments`} className='flex flex-col text-sm w-3/12 text-center items-center justify-center font-bold'>
            ...
            {
                hasDelivery
                    ? <NavLink to={`${url}/delivery`} className='flex flex-col text-sm w-3/12 text-center items-center justify-center font-bold'>
            ...
            <NavLink to={`${url}/contact`} className='flex flex-col text-sm w-3/12 text-center items-center justify-center font-bold'>
                {}
                {theme.assets.icon ? <> <img src={theme.assets.icon} height={20} width={20} /> Contactos </>
...
function Rest (props) {
    return (
        <svg viewBox="-949 951 100 125" fill='white' width={25} height={20} {...props}>
            <switch>
                <g>
