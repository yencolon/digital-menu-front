...
function MenuNavigation({ match, containerStyle, itemStyle, hasDelivery = true }) {
    ...
    return (
        <nav className="bottom-0 fixed w-full lg:hidden flex items-center justify-around flex-wrap p-1 text-white" style={containerStyle}>
            ...
            <NavLink to={`${url}`} className='flex flex-col text-sm w-3/12 text-center'>
                <HomeOutlined style={{ fontSize: '20px' }} />
                Menú
            </NavLink>

            <NavLink to={`${url}/payments`} className='flex flex-col text-sm w-3/12 text-center'>
            ...
            {
                hasDelivery ?
                <NavLink to={`${url}/delivery`} className='flex flex-col text-sm w-3/12 text-center'>
            ...
            <NavLink to={`${url}/contact`} className='flex flex-col text-sm w-3/12 text-center'>
